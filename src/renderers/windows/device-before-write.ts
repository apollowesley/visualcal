...
sendButton.addEventListener('click', (ev) => {
  ev.preventDefault();
  ipcRenderer.send(IpcChannels.device.beforeWriteString.response, { data: commandInput.value });
});

cancelButton.addEventListener('click', (ev) => {
  ev.preventDefault();
  ipcRenderer.send(IpcChannels.device.beforeWriteString.cancel, { data: commandInput.value, cancel: true });
});

ipcRenderer.on(IpcChannels.device.beforeWriteString.request, (_, args: { deviceName: string, ifaceName: string, data: string }) => {
